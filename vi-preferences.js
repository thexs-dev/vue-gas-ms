    ...
    <v-container pa-2 fluid>
      ...
      <div v-show="selected === 'map'">
        ...
        <v-layout v-if="uidata.map4vue">
          <v-select class="flex xs6 mr-3" multiple clearable :items="[...mapTypeIds, ... settings.styledMap ? ['styled'] : []]" v-model="settings.mapTypes" :label="localize('Map types')"></v-select>
          <v-text-field class="flex xs6 xmr-3" v-model="settings.styledMap" placeholder=" " append-icon="mdi-eye" @click:append="settings.styledMap = uidata.styledMap" append-outer-icon="mdi-help-circle" @click:append-outer="$open('https://www.thexs.ca/posts/styled-google-map-on-the-mapping-web-app')" :label="localize('label-styled-map')"></v-text-field>
        </v-layout>
