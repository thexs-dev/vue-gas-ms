  ...
  <v-content>
    <v-container pa-2 fluid>
      ...
      <div v-show="selected === 'layers'">
        ...
        <v-layout v-if="uidata.layerDataFileAvailable || itsme">
          <v-checkbox class="mr-3 text-no-wrap" v-model="settings.layers.datafile.enabled" :label="localize('Data File')"></v-checkbox>
          <v-select class="mr-3" :items="'GeoJSON'.split(',')" v-model="settings.layers.datafile.type" :disabled="!settings.layers.datafile.enabled" :label="localize('Type')"></v-select>
          <v-text-field class="mr-3" v-model="settings.layers.datafile.url" :disabled="!settings.layers.datafile.enabled" :label="localize('File URL')" placeholder=" "></v-text-field>
          <v-text-field class="mr-3" v-model="settings.layers.datafile.fillOpacity" type="number" step="0.01" min="0" max="1" :disabled="!settings.layers.datafile.enabled || settings.layers.datafile.type === 'Kml/Kmz'" :label="localize('label-fill-opacity')"></v-text-field>
          <v-icon @click="$open('https://www.thexs.ca/xsmapping/adding-custom-layers')">mdi-help-circle</v-icon>
        </v-layout>
